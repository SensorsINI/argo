...
if command -v systemctl >/dev/null 2>&1; then
    ...
    for service in argo_launch_standard argo_power_control argo_battery_water argo_bno085 \
        ...
        if systemctl list-unit-files "$service.service" >/dev/null 2>&1; then
            # Get timeout value (may be formatted like "15s" or "infinity" or raw microseconds)
            TIMEOUT=$(systemctl show -p TimeoutStopUSec --value "$service.service" 2>/dev/null || echo "unknown")
    ...
    systemctl list-units --type=service --state=running --no-pager 2>/dev/null | \
        ...
        while read -r unit; do
            ...
            if [ "$TIMEOUT" != "infinity" ] && [ -n "$TIMEOUT" ] && [ "$TIMEOUT" != "0" ]; then
                # Extract numeric value (timeout might be "30s" or just a number)
                TIMEOUT_NUM=$(echo "$TIMEOUT" | sed 's/[^0-9]//g')
                if [ -n "$TIMEOUT_NUM" ] && [ "$TIMEOUT_NUM" -gt 10 ]; then
                    STATE=$(systemctl is-active "$unit" 2>/dev/null || echo "unknown")
                    log_msg "  $unit: timeout=$TIMEOUT, state=$STATE"
                fi
            fi
        done
