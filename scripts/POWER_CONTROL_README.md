...
This system provides intelligent power control for the Orange Pi Zero 2W using external relays and GPIO pins, implementing safe shutdown procedures and power button monitoring.
...
### 1. Power Control Daemon (`power_control.py`)
...
### 2. Systemd Service (`power-control.service`)
...
### 3. Configuration File (`power_control_config.yaml`)
...
sudo ./install_power_control.sh
```

### Manual Installation
1. **Install systemd service**:
   ```bash
   sudo cp power-control.service /etc/systemd/system/
   sudo systemctl daemon-reload
   sudo systemctl enable power-control
   ```

2. **Make script executable**:
   ```bash
   sudo chmod +x /home/orangepi/argo/scripts/power_control.py
   ```

3. **Start the service**:
   ```bash
   sudo systemctl start power-control
   ```
...
```bash
# Show help
python3 power_control.py --help

# Normal operation
sudo ./power_control.py

# Test mode (safe - no shutdown/power control)
sudo ./power_control.py --test-mode

# Custom button press threshold
sudo ./power_control.py --threshold 2.0
```
...
sudo systemctl status power-control

# View logs
sudo journalctl -u power-control -f

# Restart service
sudo systemctl restart power-control

# Stop service
sudo systemctl stop power-control
```
...
1. **GPIO Permission Denied**
   - Ensure running as root: `sudo ./power_control.py`
...
4. **Relay Not Controlling Power**
   ...
   - Use test mode first: `sudo ./power_control.py --test-mode`

### Log Analysis
```bash
# View power control logs
sudo journalctl -u power-control --since "1 hour ago"
...
### Configuration File
Edit `power_control_config.yaml` for centralized configuration:
```yaml
power_button:
