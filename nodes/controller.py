    ...
    def timer_callback(self):
        ...
        if self.boat_state.human_controlled:
            ...
            if self.boat_state.target_heading is None:
                self.get_logger().warn("Robot control active, but no target heading. Waiting for human to set a course.", throttle_duration_sec=5)
                return
            ...
            if self.boat_state.has_critical_alerts():
                if self.boat_state.battery_low_alert:
                    self.get_logger().warn("🔋 BATTERY LOW - Autonomous control may be limited", throttle_duration_sec=10)
                if self.boat_state.saltwater_alert:
                    self.get_logger().warn("💧 SALTWATER DETECTED - Consider emergency return", throttle_duration_sec=10)
