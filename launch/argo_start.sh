...
if systemctl --user is-active --quiet argo_launch.service 2>/dev/null; then
...
if systemctl --user start argo_launch.service 2>/dev/null; then
    echo "✅ Argo launch service started successfully"
elif sudo systemctl start argo_launch.service 2>/dev/null; then
    echo "✅ Argo launch service started successfully (with sudo)"
else
    echo "❌ Failed to start argo_launch service"
